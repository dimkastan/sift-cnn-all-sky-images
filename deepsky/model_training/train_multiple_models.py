"""
Initiate from different simclr models
tune only the latest layer
"""
import torch
import sys
import os
...
from deepsky.model_training.rgb_params import  ParamsGRSCD as Config
...
model_list = sorted(glob.glob("/home/ellab4gpu/KastanWorkingDir/Dimitris-SIMCLR/SimCLR-Sequences/DeepSky-Project-Material/runs/Jan28_02-19-01_ellab4gpu-X299X-AORUS-MASTER/*.tar"))


logging.info(args.epochs)

for checkpoint in model_list:
    logging.info("Training from: \n")
    logging.info("Checkpoint = {}".format(checkpoint))
    args.pretrainedModel = checkpoint
    print("Parsing and generating training Configuration  ...\n")
    P = Config(**vars(args), logging=logging)
    epochs,\
    model,\
    optimizer ,\
    scheduler,\
